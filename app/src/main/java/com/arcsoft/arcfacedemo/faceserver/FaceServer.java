        ...
        synchronized (this) {
            if (faceEngine == null && context != null) {
                faceEngine = new FaceEngine();
                  int engineCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_VIDEO, FaceEngine.ASF_OP_0_HIGHER_EXT, 10, 1, FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);
        ...
        synchronized (this) {
            ...
            for (File featureFile : featureFiles) {
                try {
                    ...
                    fis.read(feature);
                    fis.close();
                    FaceRegisterInfo faceRegisterInfo = new FaceRegisterInfo(feature, featureFile.getName());
                    faceRegisterInfoList.add(/*new FaceRegisterInfo(feature, featureFile.getName())*/faceRegisterInfo);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
